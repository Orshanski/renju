"""Откат БЕЗ очистки TT. Копия probe_undo_raw.py с единственным отличием: после
TAKEBACK НЕ шлём YXHASHCLEAR. Чистый raw, БЕЗ серверного кода. Зоны/YXBLOCK нет.
...
Сценарий на свежем процессе:
  ...
  3) TAKEBACK до первого хода;
  4) НИКАКОЙ очистки (в этом вся суть теста);
  5) заново строим ту же линию белых, проверяем — перехватит или сольёт.

Контраст: с YXHASHCLEAR (probe_undo_raw) откат давал 100/100 перехват. Если без
очистки посыплется — TT от снятых линий травит расчёт на возвращённой позиции, и
очистка после отката обязательна (ровно её добавляет наш серверный sync_after_undo).
"""
...
# ── одна партия: построить тройку → откатить до 1-го → БЕЗ очистки → отстроить заново ─
async def one_game(echo: bool = False) -> tuple[str, list[tuple[int, int]]]:
    ...
    try:
        ...
        for w in TRIPLE:
            if w in black:  # движок занял клетку линии уже на построении (редко) — мимо цели
        ...
        # ── ФАЗА 3: НИКАКОЙ очистки (в этом весь тест) ──
...
async def main():
    print(f"ОТКАТ БЕЗ ОЧИСТКИ: тройка белых {TRIPLE} → TAKEBACK до 1-го → (без YXHASHCLEAR) → заново линия {HUMAN}")
    print("первая партия печатает реальные команды (YXHASHCLEAR отсутствует):")
    res = Counter()
    sample = {}
    for k in range(N):
