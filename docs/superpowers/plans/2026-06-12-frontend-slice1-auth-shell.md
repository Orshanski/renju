...
- **Двухстадийное ревью по задачам (Sonnet impl + Opus spec+quality):** T2/T3/T5/T6 — Approved; минорные фиксы внесены (T2: сброс 401-обработчика в setup + тесты non-JSON/204; T3: тест useAuth-вне-провайдера + `Promise<void>`; T5: `FormEvent`→`SyntheticEvent`, гард двойного сабмита; T6: cleanup `UnauthorizedBridge`). **Сознательные KEEP (документированы):** явный `navigate` в `Shell.onLogout` — самодостаточность logout (гейт — бэкстоп); `user?.username` — `?.` как страховка под гарантией гейта. **KEEP про App-интеграцию СНЯТ** (вопрос Alexey после холистик-ревью, rj-ehx): дисциплина B — про пиксели, поведение тестируемо → добавлен `App.test.tsx` (без сессии→ленивый логин; глобальный 401→бридж на /login; catch-all `*`→`/`) + регрессионный тест «тихого» старт-401 и фикс `apiMe` (`skipAuthRedirect`).
