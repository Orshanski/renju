...
Дополнить `tests/unit/test_registry.py`: **обновить `make_registry`** — передать `data_dir=tmp_path` (конструктор теперь требует его) и Path-конфиг с валидным base-TOML (для новых config-path-тестов); фейк-spawn фиксирует `config_path`. Проверить: (а) спавн партии под собранный путь; (б) **`_respawn` берёт `slot.config_path`**, а не глобальный (форс-respawn → тот же путь); (в) `_terminate` удаляет файл. `tests/integration/test_registry_live.py`: партия с nnue=off реально стартует (rate-over-N, без завязки на ход — см. rj-xv1). Реестр в этом тесте — с `cwd=settings.rapfi_config.parent` (иначе относительные веса для конфига из `data_dir` не резолвятся; `rapfi_paths`-фикстура сейчас даёт `cwd=REPO_ROOT`).
