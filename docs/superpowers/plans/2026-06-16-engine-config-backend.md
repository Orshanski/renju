...
- Test: `backend/tests/unit/test_engine_config_file.py` (новый), `backend/tests/unit/test_registry.py` (дополнить), `backend/tests/integration/test_registry_live.py` (новый live-тест nnue=off — по образцу holds-line тестов из rj-xv1, уже в файле)
...
- [ ] **Step 6: Тесты реестра (unit + live nnue=off holds-line)**

Дополнить `tests/unit/test_registry.py`: **обновить `make_registry`** — передать `data_dir=tmp_path` (конструктор теперь требует его) и Path-конфиг с валидным base-TOML (для новых config-path-тестов); фейк-spawn фиксирует `config_path`. Проверить: (а) спавн партии под собранный путь; (б) **`_respawn` берёт `slot.config_path`**, а не глобальный (форс-respawn → тот же путь); (в) `_terminate` удаляет файл. `tests/integration/test_registry_live.py`: добавить live-тест, что **собранный nnue=off-конфиг реально поднимает рабочий движок и наше вождение его не ломает** — ПО ОБРАЗЦУ уже лежащих в файле holds-line тестов rj-xv1 (`test_engine_black_holds_line_through_undo` / `test_engine_white_holds_line_through_zone_and_undo`): прогон сценария через `EngineRegistry` (compute_move + sync_after_undo + зона), исход по геометрии (`_made_five` — противник не собрал пять), НЕ по конкретному ходу движка; ОДИН прогон (на тривиальной линии движок надёжен), НЕ rate-over-N. Отличие от тех двух: реестр строится с `config_path = build_engine_config(nnue=False, ...)` (классика, без эвалюатора) и **`cwd=settings.rapfi_config.parent`** (иначе относительные classical-веса для конфига из `data_dir` не резолвятся; `rapfi_paths`-фикстура даёт `cwd=REPO_ROOT`). Это и проверяет ПОДАЧУ off-конфига: кривая сборка/cwd → движок не поднимется или поедет → тест упадёт.
