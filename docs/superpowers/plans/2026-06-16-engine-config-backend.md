...
Дополнить `upgrade()` миграции B1 (после сидов): `conn = op.get_bind()`, по каждой строке `games` распарсить JSON `controllers`; у engine-стороны дописать `strength/timeout_ms/nnue` (из засеянных уровней + `engine_settings.nnue`), записать обратно. **Источник для неизвестного/`-` уровня — конфиг НОВИЧКА** (через тот же засеянный `_levels_map["novice"]`, НЕ отдельный хардкод и НЕ пропуск): так у КАЖДОЙ партии есть полный снимок, и строгий `controller_from_json` безопасен (иначе партия без снимка уронила бы даже список). user-контроллеры пропускаем; уже-замороженные пропускаем (идемпотентность). Если игр в БД нет — цикл пуст.
**Тест бэкфилла обязателен** (на пустой БД цикл пуст → иначе не покрыт): в `tests/unit/test_migration.py` ДО `upgrade` на ревизии-перед-нашей вставить игру со старым `Engine(level_id)`-контроллером (без frozen-полей) → `alembic upgrade head` → проверить, что у engine-стороны появились `strength/timeout_ms/nnue` из сида. Доп. кейс: партия с engine `level_id="-"`/неизвестным — заморожена конфигом новичка (`strength=5/timeout_ms=1000/nnue=True`), `level_id` не переписан.
...
- Modify: `backend/app/app_factory.py` — при создании `EngineRegistry` передать `data_dir=settings.data_dir` и `cwd=settings.rapfi_config.parent` (каталог конфига; сейчас `cwd=REPO_ROOT`).
...
- Test: `backend/tests/unit/test_engine_config_file.py` (новый — валидация сгенерированного конфига), `backend/tests/unit/test_registry.py` (дополнить), `backend/tests/integration/test_registry_live.py` (live holds-line ИГРА на КАЖДОМ конфиге, что выдаёт сборщик — по образцу rj-xv1). Цепочка: сгенерировал → проверил валидность (unit) → прогнал на нём игру (live).
...
def test_nnue_off_drops_evaluator(tmp_path: Path):
    ...
    assert cfg["model"]["binary_file"] == "rapfi/Networks/classical/model210901.bin"  # путь относительный, не трогаем
    assert cfg["general"]["coord_conversion_mode"] == "none"
```

(BASE_TOML — короткий валидный фрагмент; в проде `base` = текст `engine/config.toml`.)
...
- `build_engine_config(*, nnue: bool, game_id: str, data_dir: Path, base: str) -> Path` — берёт ТЕКСТ базового TOML (`base`), при `nnue=False` УБИРАЕТ секцию `[model.evaluator]` и её `[[model.evaluator.weights]]` (оставляя `[model] binary_file`), пишет `data_dir / "engine_configs" / f"{game_id}.toml"`, возвращает путь. Каталог создавать (`mkdir parents exist_ok`).
  - **Пути весов/модели НЕ трогаем** — оставляем относительными как в шаблоне. Они резолвятся через CWD процесса: реестр спавнит партию с `cwd = каталог конфига движка` (Step 5), а `getModelFullPath` (`command.cpp:140-165`, CWD-ветка `std::filesystem::exists(modelPath)` на строках 146-148 — проверяется ВТОРОЙ, сразу после абсолютной) находит путь относительно CWD. Важно: TOML партии лежит в `data_dir/engine_configs/`, поэтому ветка «относительно каталога конфига» (152-156) веса НЕ нашла бы — рабочая именно CWD-ветка. Никаких абсолютов/переписывания (так и катится на прод — `cwd` берётся из env-конфигурируемого `RENJU_RAPFI_CONFIG`).
  - Парсинг/правка: `tomllib` для чтения + пересборка ИЛИ текстовая вырезка секции — на выбор, детерминированно.
...
- Конструктор `__init__`: принять `data_dir: Path` (сохранить `self._data_dir`). В `app_factory.py` передать `data_dir=settings.data_dir` и **`cwd=settings.rapfi_config.parent`** (вместо `REPO_ROOT`) — процесс партии спавнится с cwd=каталог конфига движка, относительные пути весов резолвятся через CWD (`getModelFullPath`, `command.cpp:140-165`, CWD-ветка 146-148).
...
- **Сборка при спавне** (`_spawn_into`): если `nnue is not None` и `slot.config_path is None` → `slot.config_path = build_engine_config(nnue=nnue, game_id=game_id, data_dir=self._data_dir, base=self._config.read_text())`. Спавн под `slot.config_path or self._config` (`None` → базовый шаблон). Существующему слоту путь НЕ пересобираем (immutable на сессию).
...
- [ ] **Step 6: Тесты реестра (unit + live holds-line ИГРА на каждом сгенерированном конфиге)**

Дополнить `tests/unit/test_registry.py`: **обновить `make_registry`** — передать `data_dir=tmp_path` (конструктор теперь требует его) и Path-конфиг с валидным base-TOML (для новых config-path-тестов); фейк-spawn фиксирует `config_path`. Проверить: (а) спавн партии под собранный путь; (б) **`_respawn` берёт `slot.config_path`**, а не глобальный (форс-respawn → тот же путь); (в) `_terminate` удаляет файл. `tests/integration/test_registry_live.py`: **полная цепочка для конфига — сгенерировал сборщиком → прогнал НА НЁМ живую игру.** Для КАЖДОГО конфига, что выдаёт `build_engine_config` под поддерживаемые настройки (параметризация ВЫХОДАМИ генератора; набор расширяем — не фиксированное число), прогнать holds-line игру ПО ОБРАЗЦУ rj-xv1 (`test_engine_black_holds_line_through_undo` / `test_engine_white_holds_line_through_zone_and_undo`): сценарий через `EngineRegistry` (compute_move + sync_after_undo + зона), исход по геометрии (`_made_five` — противник не собрал пять), НЕ по конкретному ходу движка; ОДИН прогон на конфиг (на тривиальной линии движок надёжен), НЕ rate-over-N. Реестр для каждого строится с `config_path = build_engine_config(..., base=<текст engine/config.toml>, ...)` и **`cwd=settings.rapfi_config.parent`** (иначе относительные веса для конфига из `data_dir` не резолвятся; `rapfi_paths`-фикстура даёт `cwd=REPO_ROOT`). Так проверяется ГЕНЕРАЦИЯ+ПОДАЧА: кривая сборка/cwd → движок не поднимется или поедет → тест упадёт. (Валидность самого TOML — отдельным юнитом, Step 1; здесь — что на нём реально ИГРАЕТСЯ.)

**Капстоун — САМАЯ ДЛИННАЯ цепочка (полный жизненный цикл конфига партии), обязательна:**
конфиг сгенерирован → **создание партии** замораживает снимок в Engine-контроллер (`create_game(owner_id, engine_ctl, human_color)`, B2) → **вход** (`mark_present`): реестр собирает файл из снимка под `data_dir`, процесс поднят → **прогон игры** (`compute_move` — движок отвечает, holds-line по геометрии) → **остановка** (`mark_absent` leave → `_terminate`): процесс погашен, файл удалён (assert: `Path` файла больше НЕ существует, слот ушёл) → **возобновление** (повторный `mark_present`): файл ПЕРЕсобран из ТОГО ЖЕ снимка, процесс поднят заново → **прогон игры снова** (движок снова играет корректно по геометрии). Проверяет всю механику B2+B3 разом: заморозку, сборку-при-входе, удаление-при-выходе, **регенерацию-при-возобновлении**; регенерированный конфиг идентичен исходному (снимок неизменяем, не дрейфует). Гнать через игровой слой+реестр (create_game → mark_present → compute_move → mark_absent → mark_present → compute_move) с РЕАЛЬНЫМ движком (`rapfi_paths` + `cwd=settings.rapfi_config.parent`); исход — по геометрии, не по ходу. Это и есть конечная проверка «партия живёт под своим конфигом весь цикл стоп/продолжение».
...
- **Открытые для исполнителя (детерминировать при реализации, не placeholder):** способ вырезки секции `[model.evaluator]` (tomllib+пересборка vs текст) — на выбор, с тестом on/off; точная проводка `ConfigRepository` в резолв снимка (в роутере `POST /api/games`, см. B2 Step 5). Уборка TOML-файла партии — в `_terminate` реестра (B3 Step 5), НЕ в delete-пути: файл привязан к процессу, гасится со всеми путями выхода.
