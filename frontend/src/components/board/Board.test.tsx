...
it("пустой forbidden — ни одного ✕ на доске", () => {
  renderBoard();
  expect(screen.queryAllByText("✕")).toHaveLength(0);
});

it("длинная партия: цвета по чётности, маркер на последнем (белом) камне", () => {
  renderBoard({ moves: [[7, 7], [6, 6], [8, 7], [5, 5]] as Point[] });
  expect(screen.getByTestId("stone-8-7").className).toContain("black"); // индекс 2
  expect(screen.getByTestId("last-5-5").className).toContain("lastOnWhite"); // индекс 3 — белый
});
